"""Tests for play-by-play ingestion (S007, sample-only for now — feeds S008).
...
from nfl.ingest import pbp


def _fake_frame() -> pl.DataFrame:
    return pl.DataFrame(
        {
            "game_id": ["2025_01_KC_BUF", "2025_01_KC_BUF"],
            "season": [2025, 2025],
            "week": [1, 1],
            "play_type": ["pass", "run"],
        }
...
def fake_loader(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    monkeypatch.setattr("nflreadpy.load_pbp", _load, raising=True)
    return captured


def test_fetch_pbp_adds_metadata(fake_loader: dict[str, Any]) -> None:
    out = pbp.fetch_pbp(seasons=[2025])

    assert {"ingested_at", "source"} <= set(out.columns)
    assert out["source"].to_list() == [pbp.SOURCE] * 2
    assert out["ingested_at"].dtype == pl.Datetime
    assert out.select(pl.col("ingested_at").max()).item() <= dt.datetime.now(dt.UTC)


def test_fetch_pbp_preserves_source_columns_and_rows(fake_loader: dict[str, Any]) -> None:
    out = pbp.fetch_pbp(seasons=[2025])

    assert set(_fake_frame().columns) <= set(out.columns)
    assert out.height == _fake_frame().height
    assert out["play_type"].to_list() == ["pass", "run"]


def test_fetch_pbp_requests_the_given_seasons(fake_loader: dict[str, Any]) -> None:
    pbp.fetch_pbp(seasons=[2023, 2024])
...
def test_main_writes_parquet_to_the_configured_data_dir(
    ...
    rc = pbp.main(["--season", "2025"])

    target = tmp_path / pbp.FILENAME
    assert rc == 0
